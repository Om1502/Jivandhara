...
def hel():
    ...
    conn.execute('CREATE TABLE IF NOT EXISTS users (name TEXT, addr TEXT, city TEXT, pin TEXT, bg TEXT,email TEXT UNIQUE, pass TEXT , phone INTEGER)')
   ...
   if request.method == 'POST':
      try:
         ...
         with sqlite3.connect("database.db") as con:
             cur = con.cursor()
             cur.execute("INSERT INTO users (name,addr,city,pin,bg,email,pass,phone) VALUES (?,?,?,?,?,?,?,?)",(nm,addr,city,pin,bg,email,passs,phone) )
             con.commit()
...
def bl():
    conn = sqlite3.connect('database.db')
    print("Opened database successfully")
    conn.execute('CREATE TABLE IF NOT EXISTS blood (id INTEGER PRIMARY KEY AUTOINCREMENT, type TEXT, donorname TEXT, donorsex TEXT, qty TEXT, dweight TEXT, donoremail TEXT, phone TEXT,plasma  boolean)')
    print( "Table created successfully")
    conn.close()
...
def addb():
    ...
    if request.method == 'POST':
        try:
           ...
           with sqlite3.connect("database.db") as con:
              cur = con.cursor()
              cur.execute("INSERT INTO blood (type,donorname,donorsex,qty,dweight,donoremail,phone,plasma) VALUES (?,?,?,?,?,?,?,?)",(type,donorname,donorsex,qty,dweight,email,phone,plasma) )
              con.commit()
              msg = "Record successfully added"
        except:
           con.rollback()
